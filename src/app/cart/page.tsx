          ...
          <div className="lg:col-span-2 space-y-6">
            {items.map((item, i) => (
              <div key={i} className="flex gap-6 border-b border-[#2a2a2a] pb-6">
                <div className="h-24 w-24 flex-shrink-0 bg-[#111] border border-[#2a2a2a] flex items-center justify-center">
                  <span className="text-2xl font-bold text-[#2a2a2a]">{item.product.name[0]}</span>
                </div>
                <div className="flex-1">
                  <h3 className="text-sm font-semibold text-white">{item.product.name}</h3>
                  <p className="mt-1 text-sm text-[#d4af37]">{formatPrice(item.product.price)}</p>
                  <div className="mt-2 flex items-center gap-2">
                    <button onClick={() => { const next = [...items]; next[i] = { ...next[i], quantity: Math.max(1, next[i].quantity - 1) }; setItems(next); }} className="h-8 w-8 rounded border border-[#2a2a2a] text-white">-</button>
                    <span className="w-8 text-center text-sm text-white">{item.quantity}</span>
                    <button onClick={() => { const next = [...items]; next[i] = { ...next[i], quantity: next[i].quantity + 1 }; setItems(next); }} className="h-8 w-8 rounded border border-[#2a2a2a] text-white">+</button>
                    <button onClick={() => setItems(items.filter((_, j) => j !== i))} className="ml-4 text-xs uppercase tracking-widest text-[#a0a0a0] hover:text-red-500">Remove</button>
                  </div>
                </div>
              </div>
            ))}
